...
- **bbox の長辺がフレームの 90% 以上、短辺が 20% 以上** — v7 の「小さすぎ」を機械で落とす。`fitCamera` は margin 0.08 で収めるため制約側の軸は必ず 0.926 前後まで埋まる。当初は「縦横とも 55% 以上」としていたが、仰臥位のような横長の被写体を正方形フレームに収めると縦が余るため幾何学的に必ず落ちる（`dh_hang_r` の実測は 0.999 × 0.468）
- bbox はベッドを含むため患者単体の大きさは測れない。患者が十分大きいことは `fitCamera` が患者の `framingPoints` をフレームに合わせることで構造的に保証しており、この検査はその保証が壊れていないことの裏取りにあたる
