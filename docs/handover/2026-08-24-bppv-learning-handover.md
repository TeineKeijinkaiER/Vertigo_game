...
各パターンに、誘発試験のアニメーション、眼振アニメーション、耳石置換法のアニメーション、手順と注意点を置いた。型と患側の選択には場所を取らないネイティブのコンボボックスを使う。

## 実装の配置

- `src/data/bppvLessons.ts`
  - 6パターンの表示文言、眼振、動画、手順を定義する唯一の学習用データ。
...
| 水平向地性 右 / 左 | `headroll_r` と `headroll_l` | `gufoni_geo_r` / `gufoni_geo_l`、`lempert_r` / `lempert_l` |
...
1. 375px幅で、コンボボックス・動画・眼振図がはみ出さないこと。
