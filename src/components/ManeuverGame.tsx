...
/**
 * 耳石置換法を組み立てるミニゲーム。
 * 手技を選ぶ → 患側を選ぶ → 各手順で倒す向き・頭を回す向きをイラストから選ぶ。
...
export function ManeuverGame({
  ...
  if (side === null) {
    return (
      <Win title={MANEUVER_KINDS.find((m) => m.id === kind)?.label}>
        <p className="msg small dim" style={{ margin: '0 0 8px' }}>
          患側はどちらですか。
        </p>
        <div className="menu">
  ...
  const steps = buildSteps(kind, side)
  const index = answers.length

  if (index < steps.length) {
    const step = steps[index]
    return (
      <Win title={`${MANEUVER_KINDS.find((m) => m.id === kind)?.label}　${side === 'R' ? '右' : '左'}`}>
  ...
  // 全手順を選び終えた
  const perfect = steps.every((s, i) => s.correct === answers[i])
  return (
    <Win title="手技を実施する">
      <p className="msg" style={{ margin: '0 0 10px' }}>
        {MANEUVER_KINDS.find((m) => m.id === kind)?.label}（{side === 'R' ? '右' : '左'}）を、この手順で行います。
      </p>
      <div className="stack">
        {steps.map((s, i) => (
          <div className="small" key={s.question}>
            {s.question}　→　<span className="accent">{s.options.find((o) => o.value === answers[i])?.label}</span>
          </div>
        ))}
      </div>
