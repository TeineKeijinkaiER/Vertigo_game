...
export function ExamScreen({
  ...
  const finishManeuver = (attempt: ManeuverAttempt) => {
    ...
    // 治らなかったときは「変化なし」では済まない。手技は頭位を何度も変えさせる操作で、
    // めまいの患者はそれだけで嘔気が強くなる。治らなければ苦痛だけが残る
    const text = cured
      ? `${kindLabel}（${sideLabel}）を正しい手順で施行した。\n\n直後に頭位変換試験を再検すると、眼振もめまいも誘発されない。患者は「治りました」と目を丸くしている。`
      : `${kindLabel}（${sideLabel}）を施行した。眼振もめまいも治まらない。\n\n頭位を何度も変えられた患者は、施行前より強い嘔気を訴えている。「先生、さっきより気持ち悪いです」\n\n${hint}`
