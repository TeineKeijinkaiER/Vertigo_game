...
export const case04: CaseDef = {
  ...
  explanation:
    '水平半規管BPPVは全BPPVの10〜20%を占めます。Supine Head Rollで水平眼振が誘発され、Dix-Hallpikeは陰性となるのが特徴です。向地性（下になった耳の方向へ向かう）眼振で、眼振が強い側が患側です。本例では右耳下で強い右向き眼振が出ており、右水平半規管が患側と判断できます。治療はLempert法（Barbecue回転）で、仰臥位から健側方向へ90°ずつ回していきます（患側を上にした側臥位 → 腹臥位 → 患側を下にした側臥位 → 坐位）。各体位は30〜60秒保持し、合計270〜360°回します。Gufoni法（向地性型）＝健側へ倒れて顔を下向き、でも構いません。本ゲームではどちらを選んでも正解として扱います。\n\nこの症例の本当の教育目的は、MRIをオーダーする前に禁忌を確認することです。70歳・高血圧・不整脈と並べば中枢性を心配したくなりますが、所見は典型的な頭位性で神経学的異常もありません。そして何より、この患者にMRIは撮れません。',
  mriNote:
