...
export const case01: CaseDef = {
  ...
  findings: {
    ...
    tx_rehab:
      'Brandt-Daroff運動のやり方を紙に書いて渡した。「治療の主役はEpley法で、この運動は再発したときの自己治療と、残ってしまう浮動感に対する慣らし運動です」と位置づけを説明し、再発時は再受診するよう伝えた。',
  },
  ...
  endings: {
    best:
      'Epley法を1回施行すると、眼振はすっと消えた。もう一度Dix-Hallpikeを行っても、めまいは誘発されない。\n\n「先生、世界が止まりました」\n\n患者は自分の足でしっかりと立ち上がった。再発したときのためにと渡したBrandt-Daroff運動の紙を握りしめ、娘さんと並んで帰っていく。翌週の耳鼻科外来でも、再発はなかった。',
    good:
  ...
  keyPoints: [
    ...
    '治療の主役はEpley法。Brandt-Daroff運動は再発時の自己治療・残存浮動感の慣らしであり、再発予防の効果は確立していない',
  ],
  explanation:
    '後半規管BPPVは全BPPVの80〜90%を占める最頻のBPPVで、耳石が後半規管内に遊離することで発症します。典型像は起床時・仰臥位からの起坐時・頭後傾位で誘発される回転性めまいです。Dix-Hallpike法での上向き回旋眼振は後半規管の興奮性刺激（カナリシウム型）を示し、眼振の回旋成分は患側（下にした耳側）に向かいます。治療はEpley法が第一選択で、多くは1〜2回で治癒します。再発は1年で15〜30%程度と報告されていますが、Epley法のあとの安静・体位制限に再発を減らす効果はなく、ルーチンには行いません。Brandt-Daroff運動も再発予防効果が確立しているわけではなく、Epley法が奏効しなかったとき・再発したときの自己治療、および置換後に残る浮動感に対する慣らし運動として指導します。再発時には再受診するよう伝えることのほうが実際的です。',
  mriNote:
