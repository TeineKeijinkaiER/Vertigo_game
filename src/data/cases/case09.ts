...
export const case09: CaseDef = {
  ...
  findings: {
    hx_course:
      '今朝、起床直後に突然、激しい回転性めまいと嘔吐が出現した。発症時刻ははっきりしている。めまいは持続性で、頭位変換で軽減せず、来院までの数時間ずっと続いている。立位はまったく取れない。',
    hx_assoc:
      '嘔吐を数回。「右の顔と、左の手足の感じがおかしい」と訴える。声がかすれ、水を飲むとむせる。頭痛は後頸部にやや強い。複視はない。難聴・耳鳴・耳閉感はない。',
    ...
    ex_cpss:
      'CPSS：正常（0点）。Barré徴候なし、口角下垂なし、構音障害なし。\n\n……CPSSは前循環梗塞のスクリーニングであり、後循環には不感度である。正常でも後頭蓋窩病変は除外できない。',
    ...
    ex_diplopia: '眼球運動は全方向で可能。明らかな複視の訴えはない。',
    ex_face:
  ...
  treatment: {
    required: ['tx_fluid'],
    forbidden: [
      { id: 'tx_oral', points: -10, reason: '急性期脳梗塞を内服処方で済ませてはいけない。しかも嚥下障害があり誤嚥の危険がある' },
    ],
  },
  ...
  endings: {
    best:
      'HITは陰性、眼振は方向可変性、そしてTest of Skewが陽性。HINTSは3項目とも中枢性のパターンだった。\n\n交叉性の感覚障害、Horner徴候、カーテン徴候。所見はすべて延髄外側を指していた。\n\n「後循環の脳梗塞を疑います。すぐに見てください」\n\n脳神経外科への連絡は早かった。MRI-DWIで右延髄外側の高信号を確認し、そのまま脳卒中ユニットへ。嚥下障害に対しては絶食のうえ経鼻胃管が挿入され、誤嚥性肺炎は起きなかった。\n\n3か月後、患者は杖歩行で外来に現れた。「水が普通に飲めるようになりましたよ」',
    good:
      '中枢性と判断して入院させ、画像で延髄外側梗塞が確認された。診断には到達した。\n\nただし嚥下障害の評価が遅れ、第2病日に誤嚥性肺炎を発症。抗菌薬治療で軽快したが、入院は2週間延びた。\n\n──水を飲ませてむせたら、そこで食事は止める。',
    bad:
      '診断名は外したが、「何かおかしい」という感覚で専門科に相談した。\n\n脳神経外科がMRIをオーダーし、右延髄外側の梗塞が判明。診断は間違えたが、患者を帰さなかったという一点が転帰を分けた。',
    worst:
      'CPSSは0点。麻痺もない。指鼻試験も、まあ大丈夫に見えた。\n\nあなたは「めまい症」として点滴をし、患者を帰宅させた。\n\n──その夜。\n\n患者は自宅で水を飲もうとしてむせ込み、そのまま呼吸状態が悪化した。翌朝、意識レベル低下と発熱で再搬送。両側の誤嚥性肺炎と、右延髄外側の梗塞が確認された。\n\n呼吸管理を要し、集中治療室に入った。\n\nカルテには、初診時のあなたの記載が残っている。\n「CPSS 0点、麻痺なし」\n\n──CPSSは後循環に不感度である。\n目の前の患者は、声がかすれ、水にむせ、支えなしでは立てなかった。',
  },

  keyPoints: [
    'CPSSは前循環梗塞のスクリーニングであり、後循環には不感度。0点でも後頭蓋窩病変は除外できない',
    'HINTSが有効なのはAVSのみ。HIT陰性＋方向可変性眼振＋Skew陽性は中枢性の確率が極めて高い',
    'Test of Skewは交代遮蔽で行う。遮蔽を外した眼が垂直に戻れば陽性',
    '右顔面と左半身という交叉性の感覚障害は延髄外側病変に特徴的。両側同時に触れて比べる',
    'カーテン徴候・嗄声・むせは疑核の障害を示す。嚥下評価を怠ると誤嚥性肺炎につながる',
    '失調Grade 3（支持なしでは立位保持不能）は脳卒中に対して特異度100%',
    '心房細動があれば抗凝固薬を内服していても心原性塞栓は起こり得る',
  ],
  explanation:
    'Wallenberg症候群（延髄外側症候群）は、後下小脳動脈（PICA）または椎骨動脈の閉塞による延髄外側部の梗塞です。典型像は「めまい＋交叉性感覚障害＋後部脳神経症状＋同側の失調」で、本例はすべて揃っています。\n\nこの症例の最大の教訓は、CPSSが0点であることです。CPSSは前循環梗塞を拾うスケールで、後循環には不感度です。麻痺がないこと、CPSSが正常であることは、脳卒中を否定する根拠になりません。\n\n代わりに拾うべきは、HINTSの3項目（HIT陰性・方向可変性眼振・Skew陽性）、両側同時刺激で分かる交叉性感覚障害、Horner徴候、そしてカーテン徴候と嗄声です。失調はGrade 3で、これ自体が脳卒中に対して特異度100%の所見です。\n\n嚥下障害は診断の手がかりであると同時に、そのまま生命予後に直結します。むせる患者を帰してはいけません。',
  mriNote:
    '本例は4条件すべてに該当し、画像検査の適応は明白です。第一選択は頭部MRI（DWI）で、延髄外側の高信号として描出されます。頭部CTでは出血は否定できますが、後頭蓋窩のアーチファクトによりこの病変は写りません。血管評価としてMRAで椎骨動脈の閉塞も確認できます。',
}
