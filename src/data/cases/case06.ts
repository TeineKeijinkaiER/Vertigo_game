...
export const case06: CaseDef = {
  ...
  keyPoints: [
    ...
    '治療の柱は輸液・制吐と早期の前庭リハビリ。ステロイドの有効性は確立していない',
    '前庭抑制薬は急性期の数日まで。漫然と続けると前庭代償を遅らせる',
  ],
  explanation:
    '前庭神経炎はウイルス感染に続発する前庭神経の急性炎症と考えられています。診断の鍵はHITで、患側への急速頭位変換で補償性サッケードが出現します。HINTS（HIT陽性＋方向不変眼振＋Skew陰性）が揃えば末梢性の確率が極めて高くなります。\n\n治療は輸液と制吐による支持療法が中心で、嘔気が落ち着き次第、早期に前庭リハビリを始めます。ステロイドは前庭機能の回復を早める報告はあるものの症状予後の改善は示されておらず、必須ではありません。むしろ前庭抑制薬を数日で切り上げることのほうが重要です。',
  mriNote:
