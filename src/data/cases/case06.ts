...
export const case06: CaseDef = {
  ...
  findings: {
    ...
    tx_atarax:
      'アタラックスP 1A（25mg）+ 生食50mLを15分で投与。10分ほどで「少し楽になった」と話す。……前庭抑制薬は急性期の症状緩和には有用だが、漫然と続けると前庭代償を遅らせる。数日でやめる前提で使う。',
    tx_primperan: 'プリンペラン 1A（10mg）+ 生食50mLを15分で投与。嘔気が和らぎ、水分を口にできるようになった。',
    tx_rehab:
      '前庭リハビリテーションについて説明した。嘔気が落ち着き次第、前庭抑制薬を減らしながら早期に開始する方針とした。早く動かすほど前庭代償が進むことも説明した。',
  },
  ...
  endings: {
    best:
      '輸液と制吐薬で嘔気が落ち着き、患者はようやく水を飲めるようになった。HINTSは3項目とも末梢性パターン。支持療法を続ける方針として、経過観察入院とした。\n\n第2病日には前庭抑制薬を減らし、そのぶん早く前庭リハビリを開始した。第5病日には介助なしで廊下を歩けるようになり、退院。1か月後の外来では「まっすぐ歩けます」と笑った。\n\n「あのとき先生が、ちゃんと目を見てくれたので」',
    good:
      '診断は正しく、支持療法で症状は改善した。ただし早期の前庭リハビリまでは踏み込まず、前庭抑制薬を漫然と続けてしまった。\n\n退院後も数週間ふらつきが残り、職場復帰までに1か月を要した。前庭代償には時間がかかったが、最終的には後遺症なく回復している。',
    bad:
  ...
  keyPoints: [
    ...
    '起立歩行では一方向（患側）に傾く程度。小脳失調のように四方へ崩れて転倒するわけではない',
    'ただし「倒れ方の型」と「ふらつきの強さ」は別物。支持なしで歩けないGrade 2は、型が末梢性でも4条件の「ふらつきが強い」に該当する',
    'HINTSが末梢性パターンで揃えばMRIは必須ではない。ただし不確実ならMRIを撮る',
    '治療の柱は輸液・制吐と早期の前庭リハビリ。ステロイドの有効性は確立していない（前庭機能検査の改善は示されても、症状予後の改善は示されていない）',
    '前庭抑制薬（抗ヒスタミン薬など）は急性期の数日にとどめる。漫然と続けると前庭代償を遅らせる',
  ],
  explanation:
    '前庭神経炎は、ウイルス感染に続発する前庭神経の急性炎症と考えられています。激しい持続性回転性めまいと嘔吐で患者は全く動けなくなります。診断の鍵はHead Impulse Testで、患側への急速頭位変換で補償性サッケードが出現します。HINTS（HIT陽性＋方向不変眼振＋Skew陰性）が揃えば末梢性の確率が極めて高くなります。治療は輸液と制吐薬による支持療法が中心で、嘔気が落ち着き次第、早期に前庭リハビリテーションを開始します。ステロイドは古くから使われてきましたが、カロリック検査上の前庭機能回復を早めるという報告はあるものの、症状やめまいの予後を改善するという十分なエビデンスはなく（Cochraneレビューでもエビデンス不十分と結論されています）、必須の治療ではありません。むしろ実務上重要なのは、前庭抑制薬（抗ヒスタミン薬・制吐薬）を急性期の数日にとどめ、漫然と続けないことです。前庭抑制薬は前庭代償を遅らせるため、動けるようになった時点で減量し、リハビリに切り替えます。本例では上気道感染の先行歴がウイルス性病因を支持します。',
  mriNote:
    '本例は4条件のうち2つ（高血圧というリスク因子のある突然発症／支持なしでは歩けないGrade 2のふらつき）に該当します。ふらつきは一方向で末梢性の型ではありますが、Grade 2という強さ自体は4条件の4番目を満たすと考えてください。\n\nそのうえで、HINTSが3項目とも末梢性パターンで完全に揃い、神経所見も陰性であれば、MRIは必須ではありません。逆にHINTSが不完全・不確実な場合、崩れ方が一方向にとどまらない場合、新たな神経所見が出た場合は、頭部MRI（DWI）を施行すべきです。本例でMRIを撮る判断は過剰ではありません。',
}
