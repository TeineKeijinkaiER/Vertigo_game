...
export const case05: CaseDef = {
  ...
  findings: {
    hx_course:
      '2日前の就寝時、左向きに寝返りを打った際に突然の回転性めまいが出現した。以降、仰臥位から左右いずれに頭位変換してもめまいが誘発される。1回の発作は1分程度で、BPPVにしてはやや長い。頭位依存性が高く、じっとしていればめまいはない。',
    ...
    tx_fall: '起床時の動作をゆっくり行うこと、手すりの使用を指導した。',
  },
  ...
  endings: {
    best:
      'Gufoni法（背地性型）を左に施行した。患側の左へ倒し、鼻を天井に向けて1分間保持する。\n\nもう一度Head Rollを行うと、背地性だった眼振が向地性に変わっていた。クプラから耳石が外れ、半規管腔内に落ちたのである。続けて向地性型の手技を行うと、めまいは消えた。\n\n「昨日から寝返りが怖かったんです」\n\n患者はその場で寝返りを打ってみせ、何も起きないことを確かめてから帰っていった。',
    good:
      '診断は正しかった。しかし耳石置換法まで踏み込まなかったため、患者は経過観察となって帰宅した。\n\nクプラ結石症は向地性型より自然軽快が遅く、症状は2週間ほど続いた。その間、寝返りのたびにめまいに耐えることになった。',
    bad:
      '診断が違っていた。それでも重篤な事態には至らず、患者は帰宅した。\n\n後日、耳鼻科でSupine Head Rollを行い、左のクプラ結石症と診断。Gufoni法（背地性型）で軽快した。',
    worst:
      '背地性眼振を前に、判断が定まらないまま時間だけが過ぎた。\n\n結局、めまいの原因は分からないまま患者は帰された。数日後、近医で「耳石が半規管の壁にくっついている状態です」と説明を受け、その場で治療されて治った。\n\n──Head Rollで眼振の向きと強さを左右で比べる。それだけで、患側は分かったはずだった。',
  },

  keyPoints: [
    'Head Rollで背地性眼振（上になった側へ向かう）が出れば、クプラ結石症を考える',
    '背地性型の患側判定は向地性型と逆。眼振が「弱い」側が患側になる',
    '背地性型は持続が長く（40〜60秒）、疲労性に乏しいのが向地性型との違い',
    '治療はGufoni法（背地性型）＝患側に倒れて顔を上（天井）に向ける',
    '背地性水平眼振は中枢性疾患でも出現しうる。神経所見と失調の確認を省略しない',
  ],
  explanation:
    'クプラ結石症は、耳石が半規管腔内を遊離しているのではなく、膨大部のクプラに付着している状態です。このため頭位変換で背地性眼振が出現し、向地性型に比べて持続時間が長く疲労性に乏しくなります。患側判定は向地性型と逆で、患側を下にしたときの眼振が弱くなります。本例では左耳下で眼振が弱いため、左が患側です。\n\n治療のGufoni法（背地性型）では、患側に倒れて顔を上に向けることでクプラから耳石を外し、向地性型に変換することを目指します。手技後に眼振が向地性へ変われば成功で、続けて向地性型の手技を行います。\n\n重要なのは、背地性水平眼振は後循環梗塞などの中枢性疾患でも出現しうる点です。本例は神経所見が陰性で失調もGrade 0のため末梢性が優位ですが、確認を省略してよい理由にはなりません。',
  mriNote:
    '背地性眼振は末梢性と決めつけられない所見です。HOWTOの4条件のうち「眼振が末梢性として矛盾する」に該当しうるため、画像検査を考慮する判断は妥当です。ただし本例は神経所見陰性・失調Grade 0・聴力正常であり、撮らずに耳石置換法へ進む判断も誤りではありません。',
}
