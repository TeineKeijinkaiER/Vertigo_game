...
export const case05: CaseDef = {
  ...
  findings: {
    hx_course:
      '2日前の就寝時、左向きに寝返りを打った際に回転性めまいが出現。以降、仰臥位から左右いずれに頭位変換してもめまいが誘発される。「どちらかというと、右を向いたときのほうがつらいです」。1回の発作は1分程度で、BPPVにしてはやや長い。じっとしていればめまいはない。',
    ...
    eye_roll_r:
      '右耳下（右側臥位）：強い水平背地性眼振（患者から見て左向き＝上になった側へ向かう）を誘発。頭位を保っている間、1分近くにわたって続き、疲労性に乏しい。',
    eye_roll_l:
  ...
  keyPoints: [
    ...
    '背地性の患側判定は向地性と逆。眼振が「弱い」側が患側。症状が強い側にひきずられない',
