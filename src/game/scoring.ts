...
export function scoreGame(c: CaseDef, s: GameState): ScoreResult {
  ...
  if (c.maneuver) {
    ...
    // 適応のない患者に行った手技には採点行を作らない。得るものがなく害だけが残るので、下で減点する。
  }

  /*
   * 治らなかった耳石置換法は減点する。手技は頭位を何度も変えさせる操作で、
   * めまいの患者はそれだけで嘔気が強くなる。治って初めて、その苦痛に見合う。
   *
   * 型を誤った場合も、患側を誤った場合も、型と患側は正しいが手順を崩した場合も、
   * そして適応のない患者に行った場合も、等しく「治らずに苦しめただけ」になる。
   * maneuverImprovedBppv は耳石置換法で症状が改善したことを表すので、その否定が該当する。
   */
  if (attempt && !maneuverImprovedBppv) {
    deductions.push({
      label: '耳石置換法',
      points: -4,
      reason: '耳石置換法で症状は改善せず、頭位変換を繰り返した分だけ嘔気を強めています',
    })
  }
