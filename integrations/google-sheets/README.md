...
**すでに UTC で記録されてしまった行は、Apps Script を貼り替えただけでは直らない。**
スクリプトエディタで `fixExistingTimestampsToJst` を1回実行すると、既存行の
UTC 表記（`2026-08-25T16:05:09.000Z` のような形）を日本時刻へ書き換える。
すでに日本時刻で入っている行は対象外なので、二重にずれることはない。

新しい時刻でアプリから記録するには、Apps Script の再デプロイに加えて
**アプリ側（GitHub Pages）の再デプロイも必要**。アプリを更新するまでの間は、
古いアプリが送ってくる UTC を Apps Script 側で日本時刻へ直して書き込む。
